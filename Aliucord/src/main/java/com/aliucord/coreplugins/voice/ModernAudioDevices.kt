...
internal object ModernAudioDevices {
    ...
    // Port of the plugin FixBluetoothAudioQuality by oSumAtrIX
    // https://github.com/oSumAtrIX/aliucord-plugins/blob/main/plugins/kotlin/FixBluetoothAudioQuality
    private const val ACTIVATE_DEVICE_METHOD_NAME = "b"

    // Hooked on compatibility mode
    private const val SET_COMMUNICATION_MODE_ON_METHOD_NAME = "h"

    // Hooked on compatibility mode
    private const val START_BLUETOOTH_SCO_METHOD_NAME = "j"
    ...
    fun register(patcher: PatcherAPI) {
        ...
        patcher.before<DiscordAudioManager>(
            SET_COMMUNICATION_MODE_ON_METHOD_NAME,
            Boolean::class.javaPrimitiveType!!,
        ) { param ->
            if (VoiceChatFixSettings.hqBluetooth && VoiceChatFixSettings.hqBluetoothCompat) {
                logger.debug("setCommunicationModeOn(): suppressed by HQ Bluetooth compatibility mode")
                param.result = null
            }
        }

        patcher.before<DiscordAudioManager>(START_BLUETOOTH_SCO_METHOD_NAME) { param ->
            if (VoiceChatFixSettings.hqBluetooth && VoiceChatFixSettings.hqBluetoothCompat) {
                logger.debug("startBluetoothSco(): suppressed by HQ Bluetooth compatibility mode")
                param.result = null
            }
        }

        // Stock b()/activateDevice() routes Bluetooth through SCO only
        // modern devices use setCommunicationDevice instead
        // classic devices (has SCO) keep the stock path
        patcher.before<DiscordAudioManager>(
            ACTIVATE_DEVICE_METHOD_NAME,
            DeviceTypes::class.java,
        ) { (param, type: DeviceTypes) ->
            if (type == DeviceTypes.BLUETOOTH_HEADSET && VoiceChatFixSettings.hqBluetooth) {
                logger.info("activateDevice(BT): HQ Bluetooth on, skipping SCO & keeping A2DP")
                k()
                e.mode = AudioManager.MODE_NORMAL
                hqRouted = true
            ...
            if (hqRouted) {
                ...
                synchronized(i) {
                    if (D) e.mode = AudioManager.MODE_IN_COMMUNICATION
                }

                logger.debug("activateDevice($type): restored communication mode after HQ Bluetooth")
            }
            ...
            if (type != DeviceTypes.BLUETOOTH_HEADSET) {
                ...
                e.communicationDevice?.takeIf { it.type in MODERN_BT_TYPES }?.let {
                    logger.info("activateDevice($type): clearing modern comm device ${it.formatName()} for non-BT activation")
            ...
            if (e.hasClassicSco()) {
                logger.debug("activateDevice(BT): classic SCO device present, keeping stock SCO path")
                return@before
            }

            val ble = e.modernBtDevice() ?: run {
                logger.warn("activateDevice(BT): no classic SCO and no modern BT device, letting stock path try")
                return@before
            }

            synchronized(i) {
                if (!D) {
                    logger.debug("activateDevice(BT): not in communication mode, skipping modern routing")
                    return@before
                }
            }

            logger.info("activateDevice(BT): routing via setCommunicationDevice to ${ble.formatName()} type=${ble.type}")
            k()

            if (e.communicationDevice?.type == AudioDeviceInfo.TYPE_BUILTIN_SPEAKER) {
                logger.debug("activateDevice(BT): clearing builtin speaker communication device first")
                e.clearCommunicationDevice()
            }
